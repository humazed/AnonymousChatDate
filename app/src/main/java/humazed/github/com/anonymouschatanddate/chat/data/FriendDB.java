...
public final class FriendDB {
    ...
    public long addFriend(Friend friend) {
        ...
        values.put(FeedEntry.COLUMN_NAME_ID, friend.getId());
        values.put(FeedEntry.COLUMN_NAME_NAME, friend.getName());
        values.put(FeedEntry.COLUMN_NAME_EMAIL, friend.getEmail());
        values.put(FeedEntry.COLUMN_NAME_ID_ROOM, friend.getIdRoom());
        values.put(FeedEntry.COLUMN_NAME_AVATA, friend.getAvata());
    ...
    public ListFriend getListFriend() {
        ...
        try (Cursor cursor = db.rawQuery("select * from " + FeedEntry.TABLE_NAME, null)) {
            while (cursor.moveToNext()) {
                Friend friend = new Friend();
                friend.setId(cursor.getString(0));
                friend.setName(cursor.getString(1));
                friend.setEmail(cursor.getString(2));
                friend.setIdRoom(cursor.getString(3));
                friend.setAvata(cursor.getString(4));
